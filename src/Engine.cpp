...
namespace karapo {
	...
	ProgramInterface Default_ProgramInterface = {
		...
		.GetEntityByFuncWithBase = [](std::function<bool(std::shared_ptr<Entity>)> cond, const std::shared_ptr<Entity>& Base) { return Program::Instance().entity_manager.GetEntity(cond, Base); },
		.ActivateEntity = [](const std::wstring& Name) { Program::Instance().entity_manager.Activate(Name); },
		.DeactivateEntity = [](const std::wstring& Name) { Program::Instance().entity_manager.Deactivate(Name); },
		.FreezeEntity = [](const std::wstring& Name) { Program::Instance().entity_manager.Freeze(Name); },
		.DefrostEntity = [](const std::wstring& Name) { Program::Instance().entity_manager.Defrost(Name); },
